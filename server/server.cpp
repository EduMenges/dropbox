...
[[noreturn]] void dropbox::Server::MainLoop() const {
    ...
    while (true) {
        ...
        const int kSyncSocket   = accept(kReceiverSocket, reinterpret_cast<sockaddr*>(&client_address), &client_length);

        if (kHeaderSocket == kInvalidSocket) {
            if (kFileSocket == kInvalidSocket) {
                close(kHeaderSocket);
            }
            if (kSyncSocket == kInvalidSocket) {
                close(kHeaderSocket);
            }
            ...
            std::thread new_client_thread(
                [](int header_socket, int file_socket, int sync_socket) {
                    try {
                        ClientHandler(header_socket, file_socket, sync_socket).MainLoop();
                ...
                kHeaderSocket, kFileSocket, kSyncSocket);
